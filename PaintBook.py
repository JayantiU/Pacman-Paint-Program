#---Pacman PaintBook---#
#---Jayanti Upadhyay---#
#This is a paint program written with pygame. It is similar to Microsoft Paint.
#It allows the user to draw simple pictures with the mouse and save it to a file.

#-----------------------Extra Features-----------------------#
# 1. Undo/redo; can use either by clicking icons or using ctrl z or ctrl y, respectivly
# 2. Use ctrl s and ctrl o for saving and opening images
# 3. 8 stickers (instead of just 6)
# 4. Text tool with bold, italics or no bold or italics options. You can slect and deselect bold and italic options by clicking and unclicking the icons. Used flags to control this
# 5. The edges are fixed for unfilled rectangle, unfilled ellipse works and polygon tool(filled and unfilled) exists and works
# 6. Spray Paint,paint brush, marker tools + paint brush and eraser with smooth circles+ marker uses RGBA colors. Pencil uses aaline to make it appear smoother
# 7. Filters in black and white and negative
# 8. If tool is clicked, then tool is highlighted and hover effect
# 9. Color drop (where you click on canvas to choose color)
# 10. Stickers, shapes and text are resizable as well as all drawing tools except pencil
# 11. Rotate canvas clockwise or counterclockwise and flip vertically or horozontially.
# 12. Play and pause buttons for the music to play or pause when clicked  
#If you click on play, the icon changes to pause and music plays and if you click on pause, the icon changes to play and music pauses
# 15. Cut, copy and paste tools.
# 16. Fill entire canvas with color selected; color fill.
# 17. Background music
# 18. Trash icon where it fills screen with white and you can restart your drawing 
# 19. When you close the program, it asks if you've saved your work or not. Used messagebox.askyesnocancel from tkinter.
# If cancel, program continues. If yes, save icon from tkinter pops up and you can save your work. Otherwise, program closes. 
### 20. Instruction for each each tool and size is displayed
#-----------------------Attention To Detail-----------------------#
# 1. Default tool is pencil so when program opens, the user can click on canvas and draw with pencil
# 2. If tool is selected,it can only be used on canvas and not anywhere else; things are properly clipped to canvas
# 3. User is told that scrolling can change size and that they should click to use tool. Insutrctions are displayed for each tool + size of each tool is displayed
# 4. Pixel location is shown on canvas
# 5. Each icon is spaced evenly apart. The icons are 40 by 40 pixels large and the icons placed on the tab are 50 by 50 pixels large. 
# 6. There are borders around the canvas, subtitle bar, side bar and color spectrum
# 7. The screen is centered 
# 8. Two images are blitted for the background to make it not look pixalated.
# 9. Whatever file name user inputs when saving or whatever file is opened is the caption for the title bar. Otherwise, the caption is: Untitled-Pacman Paintbook
...
#---FUNCTIONS---#
def save():
    paint=screen.subsurface(canvasRect).copy() #takes a screenshot of the canvas which is the image that'll be saved
    result = filedialog.asksaveasfilename(filetypes = [("PNG", "*.png")])
    if result!='': #Checks if the user did not enter nothing, so if they entered something, then the image is saved and what they enter becomes the caption
        image.save(paint,'%s.png'%result)
        display.set_caption(result[result.rfind('/')+1:]+'-Pacman Paintbook')
def load():
    result = filedialog.askopenfilename(filetypes = [("PNG", "*.png"),("JPG","*.jpg"),("BMP","*.bmp")])
    if result!='': #checks if the user selected a file
        file=transform.scale(image.load(result),(795,610)) #the size changes so that it's as large as the canvas
        screen.blit(file,(canvasRect))
        display.set_caption(result[result.rfind('/')+1:]+'-Pacman Paintbook')
        undo.append(screen.subsurface(canvasRect).copy()) #With the undo algoritm, it only takes a screenshot once you release the mouse on the canvas.
#        When you open a file, this doesn't occur so it doesn't take a screenshot. So, I took a screenshot here.
def undo_drawing():
    if len(undo)>1: #checks if there is at least two elements in the list
        redo.append(undo[-1]) #the last element in the undo list is moved to the redo list
        del(undo[-1]) #that element is deleted from the undo list
        screen.subsurface(canvasRect).blit(undo[-1],(0,0)) #then the second last image from the orginal list (before deleting the last image in the above line)is blitted onto the surface (canvasRect)
def redo_drawing():
    if len(redo)>0: #checks if there is at least one element in the list
        screen.subsurface(canvasRect).blit(redo[-1],(0,0)) #the last image is blitted onto the suface
        undo.append(redo[-1]) #then it's moved to the undo list
        del(redo[-1]) #and it's removed from the redo list
...
stickers='' #stores which sticker is selected
shapes='' #stores which shape is selected
effect='' #stores which filter/effect is chosen
text='' #stores the text options (bold, underline, italics, etc.)
select='' #stores if user clicked cut, copy or paste 
...
copied=False #checks if user copied an area. Starts as False
cutting=False #checks if user cut an area. Starts as False
copy1Rect= Rect(0,0,0,0) #starting value of the rect oject that user decided to copy
cut1Rect=  Rect(0,0,0,0) #starting value of the rect object user decides to cut.

points=[] #stores the points on the canvas the user clicked to create polygon connecting those points.
...
#COLOR DROP pic/rect
colordropRect= Rect(123,5,40,40)
n_colordropPic=transform.scale(image. load('images/normal/n_colordrop.png'),(40,40))
#SAVE pic/rect 
...
while running:
    ...
    if tools=='Select':
        ...
        if select=='cut':
            screen.blit(c_cutPic,(95,125))
            #displaying instruction for cut
            place=comicFont.render("Right click to place",True,(0,0,255))
            screen.blit(place,(95,195))
        else:
            draw.rect(screen,(234,234,234),(95,195,190,45)) #if polygon is not selected, then show nothing at this place.
    ...
    if canvasRect.collidepoint(mx,my):
        ...
        #shapes (if the user selects a shape option and then clicks on the canvas, the shape is drawn on the canvas where user clicked)
            ...
            if shapes=='rect': #unfilled rectangle
                line=(size//2)-1 #this how much more length is added on to the lines below to fix the weird edges.
        ...
        #select (if user clicks on one of the options(cut,copy,paste) then it cuts or copies area on canvas chosen by user and paste on place cliked by user on canvas))
            ...
            if copied: #if an area has been copied
                screen.subsurface(canvasRect).blit(pic,(0,0))
            ...
            if mb[2]==1:
                if cutting: #if an area has been cut
                    screen.subsurface(canvasRect).blit(pic,(0,0))
